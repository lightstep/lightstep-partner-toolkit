...
export class LaunchDarklyNodeServerPlugin extends BasePlugin<typeof ldTypes> {
  ...
  private _getVariationPatch(
    original: Function) {
    const instrumentation = this;
    instrumentation._logger.debug('Patching variation function');
    return async function variation(this: any, opts?: any) {
      const span = instrumentation._tracer.startSpan('launchdarkly - variation', {
        kind: SpanKind.CLIENT, attributes: { component: LaunchDarklyNodeServerPlugin.COMPONENT_SHORT } });
  ...
  private _getInitPatch(
    original: (options?: any) => any
  ) {
    const instrumentation = this;
...
export const plugin = new LaunchDarklyNodeServerPlugin(LaunchDarklyNodeServerPlugin.COMPONENT);
